...
class DownlinkPayloadFecTestSingle:
    ...
    def sample(self):
        ...
        signal = binaryToNrz(data)
        noise = realNoise(self.dev, len(signal))
        signal_rx = signal + noise
        
        phy_payload_rx = parseCodedPayloadSingleBurst(self.fec_codec, self.interleaver, signal_rx, self.phy_payload_length, self.burst_type)
...
class DownlinkPayloadFecTestMulti:
...
def plotFecPayloadSingle(eb_n0_arr, n):
    bers_arr_long = numpy.empty([len(structs.BURST_TYPES_DOWNLINK_SINGLE_BURST), len(eb_n0_arr)])
    bers_arr_short = numpy.empty([len(structs.BURST_TYPES_DOWNLINK_SINGLE_BURST), len(eb_n0_arr)])
    for z, burst_type in enumerate(structs.BURST_TYPES_DOWNLINK_SINGLE_BURST):
        bers_long = bers_arr_long[z]
        bers_short = bers_arr_short[z]
        for x, eb_n0 in enumerate(eb_n0_arr):
            bers_long[x] = DownlinkPayloadFecTestSingle(eb_n0, 255, burst_type).test(n)
            bers_short[x] = DownlinkPayloadFecTestSingle(eb_n0, 5, burst_type).test(n)
    
    fig, axes = pyplot.subplots(1, 2)
    plotMultiple(axes[0], eb_n0_arr, bers_arr_long, "Pogostost napake v načinu enkratne oddaje\nnavzdolnje povezave (255 zlogov vsebine)", r"$E_b/N_0 [dB]$", r"BER", True)
    plotMultiple(axes[1], eb_n0_arr, bers_arr_short, "Pogostost napake v načinu enkratne oddaje\nnavzdolnje povezave (5 zlogov vsebine)", r"$E_b/N_0 [dB]$", r"BER", True)
...
def plotFecPayloadMulti(eb_n0_arr, n):
    ...
    for z, received_count in enumerate(range(1, 3 + 1)):
        ...
        bers_short = bers_arr_short[z]
        for x, eb_n0 in enumerate(eb_n0_arr):
            bers_long[x] = DownlinkPayloadFecTestMulti(eb_n0, 255, received_count).test(n)
            bers_short[x] = DownlinkPayloadFecTestMulti(eb_n0, 5, received_count).test(n)
...
def plotFecDirect(eb_n0_arr, n):
    ...
    for z, generator_index in enumerate(range(5)):
        ...
        bers_short = bers_arr_short[z]
        for x, eb_n0 in enumerate(eb_n0_arr):
            bers_long[x] = DownlinkFecTestDirect(eb_n0, 255 * 8, generator_index).test(n)
            bers_short[x] = DownlinkFecTestDirect(eb_n0, 5 * 8, generator_index).test(n)
...
#plotFecPayloadSingle(numpy.linspace(-10, 10, 21), 5000)
plotFecPayloadMulti(numpy.linspace(-10, 10, 21), 5000)
