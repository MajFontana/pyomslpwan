...
def constructLaurentPulses(phi, L, h, SPS):
    PHI = numpy.pi * h
    J = numpy.exp(1j * PHI)
    S = numpy.sin(PHI)
    M = 2 ** (L - 1)

    psi = numpy.concatenate([phi, phi[::-1]])

    S_0 = numpy.sin(psi) / S
    S_n = [numpy.zeros((4 * L - 1) * SPS) for _ in range(2 * L)]
    for n in range(2 * L):
        S_n[n][-(n + 2 * L) * SPS:(4 * L - 1) * SPS - n * SPS] = S_0

    Ks = numpy.array(list(range(M)))
    alpha_K = [numpy.array(bitstring.Bits(uint=K, length=L - 1))[::-1] for K in Ks]

    L_K = [min([L * (2  - alpha_K[K][i - 1]) - i for i in range(1, L)]) for K in Ks]

    C_K = []
    for K in Ks:
        prod = S_n[0].copy()[-2 * L * SPS:-(2 * L - L_K[K]) * SPS]
        for i in range(1, L):
            s = S_n[i + L * alpha_K[K][i - 1]]
            s = s[-2 * L * SPS:-(2 * L - L_K[K]) * SPS]
            prod *= s
        C_K.append(prod)

    return C_K

def constructLaurentPulsesGmsk(BT, L, SPS):
    tx_pulse = GmskModulator(BT, L, SPS).modulate(numpy.ones(1), True)
    phi = numpy.unwrap(numpy.angle(tx_pulse))
    return constructLaurentPulses(phi, L, 1 / 2, SPS)